        ...
        if (aktivePhaseType === 'normal'){
            ...
            if (naechsteNiedrigphase.state){
                if(naechsteNiedrigphase.startzeit < endZeitBatterie){
                    ...
                    const startTime = naechsteNiedrigphase.startzeit.toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit', hour12: false }) + ' Uhr';
                    const endeTime = naechsteNiedrigphase.endzeit.toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit', hour12: false }) + ' Uhr';
...
function bestLoadTime(dateStartTime, dateEndTime, ladezeit_h) {
    try {
        ...
        // Suche nach dem günstigsten Ladezeitblock
        datenTibberLink48h.forEach((entry, i) => {
            const startTime = new Date(entry.startsAt);

            // Ignoriere Daten außerhalb des gültigen Zeitraums
            if (startTime < dateStartTime || startTime >= dateEndTime) return;

            // Stelle sicher, dass der Block nicht über das Array hinausgeht
            if (i + ladezeit_h > datenTibberLink48h.length) return;

            // Berechne den Gesamtpreis für den Ladezeitblock
            const blockPreis = datenTibberLink48h
                .slice(i, i + ladezeit_h)
                .reduce((sum, block) => sum + block.total, 0);

            // Aktualisiere den günstigsten Block, falls ein besserer gefunden wurde
        ...
        if (billigsteZeit) {
            log(`Günstigste Ladezeit gefunden: ${billigsteZeit}, Durchschnittspreis: ${billigsterBlockPreis / ladezeit_h}`, 'info');
            return billigsteZeit;
        } else {
            log(`function bestLoadTime: Kein passender Ladezeitblock gefunden`, 'error');
            return null;
        }
    } catch (error) {
...
on({id: sID_Power_Grid, change: "ne"}, async function (obj){
    ...
    const differenz = obj.state.val - 22000;
